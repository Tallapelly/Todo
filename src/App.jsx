    ...
    <>
      ...
      <div className="md:container mx:3 md:mx-auto my-5 rounded-xl p-5 bg-violet-100 min-h-[80vh] md:w-[35%]">
        ...
        <div className="addTodo my-5 flex flex-col gap-4">
          ...
          <button onClick={handleAdd} disabled={todo.length<=3} className='bg-violet-800 hover:bg-violet-950 mx-2 p-4 py-2 rounded-full disabled:bg-slate-500 font-bold text-sm text-white  '>Save</button>
          </div>
        </div>
          ...
          {todos.map(item => {
            return (showFinished || !item.isCompleted) && <div key={item.id} className="todo flex  my-3 justify-between">
              ...
              <div className="buttons flex h-full">
                <button onClick={(e)=>handleEdit(e,item.id)} className='bg-violet-800 hover:bg-violet-950 p-2 py-1 font-bold text-sm text-white rounded-md mx-1'><FaEdit/></button>
